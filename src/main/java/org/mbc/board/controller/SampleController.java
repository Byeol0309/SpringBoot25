...
public class SampleController {
    // 컨트롤러는 url생성과 프론트를 연결하는 부분으로 과거의 servlet-context.xml과 같은 역할을 한다.
    @GetMapping("/hello") //http://192.168.111.105:80/hello -> void -> hello.html
   public void hello(Model model){
       log.info("SampleController.hello메서드실행....");
       model.addAttribute("msg","안녕하세요 자바 종점입니다!" );
   }
   ...
   class SampleDTO{
    ...
    public void ex2(Model model){
        log.info("SampleController.ex2메서드 실행........");
        ...
        model.addAttribute("List", strList); //65행에 만들어진 객체값
        model.addAttribute("map", map);      //70행에 만들어진 객체값
