...
public class BoardSearchImpl extends QuerydslRepositorySupport implements BoardSearch {
    ...
    public Page<Board> search1(Pageable pageable) {
        // 쿼리dsl로 다중검색용 코드추가
        // 쿼리dsl의 목적은 type기반으로 코드를 이용한다. -> Q도메인 클래스
        ...
        // ( )를 사용하면 선행되기때문에 BooleanBuilder가 이 역할을 한다.
        ...
        query.where(board.bno.gt(0L)); // pk를 이용해서 빠른 검색 where이 추가되면 and조건으로 처리
        ...
        long count = query.fetchCount();  // 검색후에 게시물수 파악용
    ...
    public Page<Board> searchAll(String[] types, String keyword, Pageable pageable) {
        ...
        QBoard board = QBoard.board; // 쿼리 dsl객체 생성
        JPQLQuery<Board> query = from(board); // select * from board

        // 프론트에서 검색 폼에 keyword가 비었을 경우, 있을 경우
        ...
        query.where(board.bno.gt(0L)); // pk를 활용해서 인덱싱처리용 코드
        // where (title or content writer) and bno > 0L

        this.getQuerydsl().applyPagination(pageable, query); // 페이징 처리용 코드 + 쿼리문

        // Page<t> 클래스는 3가지 리턴타입을 만들어준다.
        List<Board> list = query.fetch();  // 쿼리문실행
