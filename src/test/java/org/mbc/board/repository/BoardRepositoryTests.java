...
public class BoardRepositoryTests {
    ...
    public void testInsert(){
        //데이터베이스에 데이터 주입(c) 테스트코드
        IntStream.rangeClosed(1,100).forEach( i -> {
            // i변수에 1-99까지 100개의 정수를 반복해서 생성
           ...
           // log.info((board));
            Board result = boardRepository.save(board); // 데이터베이스에 기록하는 코드
                            // .save메서드는 jpa에서 상속한 메서드로  값을 저장하는 용도로 사용
                            // 이미 값이 있으면 update를 진행한다.
            log.info("게시물 번호 : " + result.getBno() + "게시물의 제목" + result.getTitle());
                } //forEach문 종료
        ); //IntStream.종료
    ...
    public void testUpdate(){
        ...
        boardRepository.save(board); // .save메서드는 (pk)없으면 insert, (pk)있으면 update한다.
         //Hibernate:
        //    select
        //        b1_0.bno,
        //        b1_0.content,
    ...
    public void testPaging(){
        ...
        // 전체리스트에 페이징과 정렬 기법도 추가해보자.
        Pageable pageable = PageRequest.of(0,10, Sort.by("bno").descending());
        //                                          시작번호, 페이지당 데이터 개수
        // Sort.by 번호를 기준으로 내림차순 정렬
        //Hibernate:
        //    select
        //        b1_0.bno,
        //        b1_0.content,
        ...
        //Hibernate:
        //    select
        //        count(b1_0.bno) board전체 리스트수를 알아온다.
        //    from
        //        board b1_0

        Page<Board> result = boardRepository.findAll(pageable);
        // 1장의 종이에 Board 객체를 가지고 있는 결과는 result에 담긴다.
        // Page 클래스는 다음 페이지 존재 여부, 이전 페이지 존재 여부, 전체 데이터 갯수, 등등등을 계산을 한다.

        log.info("전체 게시물 수 : " + result.getTotalElements()); //99
        log.info("총 페이지수 : " + result.getTotalPages());      //10
    ...
    public void testSearch1(){
        ...
        Page<Board> result =  boardRepository.search1(pageable); // 페이징기법을 사용해서 title=1 값을 찾아오나?
        ...
        //    order by
        //        b1_0.bno desc
        //    limit                 // this.getQuerydsl().applyPagination(pageable,query);
        //        ?, ?
    ...
    public void testSearchAll(){
        ...
        String[] types = {"t", "w" }; // 검색조건
        String keyword = "10";         // 검색단어
        Pageable pageable = PageRequest.of(0,10, Sort.by("bno").descending());
